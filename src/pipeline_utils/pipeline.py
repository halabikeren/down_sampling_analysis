...
class Pipeline:
    ...
    def __init__(self, pipeline_input: PipelineInput):
        ...
        if pipeline_input.tree_path:
            BaseTools.simplify_names(
                pipeline_input.tree_path, self.tree_path, self.new_to_orig_names_map
            )
    ...
    def align_sampled_data(
        ...
        if os.path.exists(
            self.samples_info[fraction][method.value]["aligned_sequence_data_path"]
        ) and os.path.getsize(
            self.samples_info[fraction][method.value]["aligned_sequence_data_path"]
        ) >= os.path.getsize(
            ...
            if pipeline_input.use_full_alignment_in_sample:
                ...
                sampled_aligned_sequence_records = [
                    record
                    for record in all_aligned_sequence_records
    ...
    def build_sampled_tree(
        ...
        if (
            os.path.exists(self.samples_info[fraction][method.value]["tree_path"])
            and os.path.getsize(self.samples_info[fraction][method.value]["tree_path"])
            > 0
        ):
            logger.info(
            ...
            if pipeline_input.use_full_tree_in_sample:
                ...
                full_tree.write(
                    outfile=self.samples_info[fraction][method.value]["tree_path"]
                )
            else:
                logger.info(
    ...
    def generate_samples(self, pipeline_input: PipelineInput):
        ...
        for method in pipeline_input.sampling_methods:
            sampler_instance = method_to_callable[method.value](
                ...
                exclude_a_ref_sequence=pipeline_input.exclude_ref_seq,
            )
            for fraction in pipeline_input.sampling_fractions:
                self.samples_info[fraction][method.value][
                    "unaligned_sequence_data_path"
        ...
        df = pd.DataFrame(
            columns=["sampling_fraction", "method_1", "method_2", "overlap_fraction"]
        )
        for fraction in pipeline_input.sampling_fractions:
            method_to_sample = dict()
            for method in pipeline_input.sampling_methods:
                sample_path = self.samples_info[fraction][method.value][
                    "unaligned_sequence_data_path"
                ]
                method_to_sample[method.value] = [
                    record.name for record in list(SeqIO.parse(sample_path, "fasta"))
                ]
            for (method_1, method_2) in [
                pair
                for pair in itertools.combinations(list(method_to_sample.keys()), 2)
            ]:
                overlap_fraction = len(
                    [
                        record
                        for record in method_to_sample[method_1]
                        if record in method_to_sample[method_2]
                    ]
                ) / len(method_to_sample[method_1])
                df_record = {
    ...
    def execute_programs(self, pipeline_input: PipelineInput):
        ...
        for program_name in pipeline_input.programs:
            ...
            if pipeline_input.exec_on_full_data:
                ...
                full_data_program_aux_dir = (
                    f"{program_dir}/full_data_{program_name.value}_aux/"
                )
        ...
        for program_name in pipeline_input.programs:
            ...
            if pipeline_input.exec_on_full_data:
                try:
                    full_program_output = program_to_full_data_output[
                        program_name.value
                    ]
                    full_data_result = program_instance.parse_output(
                        output_path=full_program_output,
                        job_output_dir=full_data_program_aux_dir,
                    )
            ...
            for fraction in self.samples_info:
                for method_name in self.samples_info[fraction]:
                    ...
                    job_output_dir = self.samples_info[fraction][method_name][
                        "programs_performance"
                    ][program_name.value]["aux_dir"]
                    if os.path.exists(job_output_dir):
                        self.samples_info[fraction][method_name][
                            "programs_performance"
                        ][program_name.value]["result"].update(
                            program_instance.parse_output(
                    ...
                    if pipeline_input.exec_on_full_data:
                        self.samples_info[fraction][method_name][
                            "programs_performance"
                        ][program_name.value]["full_data_result"] = full_data_result

                    if (
                        pipeline_input.reference_data_paths
                        and program_name.value in pipeline_input.reference_data_paths
                    ):
                        self.samples_info[fraction][method_name][
                            "programs_performance"
                        ][program_name.value][
                            "reference_data"
                        ] = program_instance.parse_reference_data(
    ...
    def plot_error(
        ...
        for fraction in pipeline_input.sampling_fractions:
            for method in pipeline_input.sampling_methods:
                result_data = self.samples_info[fraction][method.value][
                    "programs_performance"
                ][program_name.value]
                ...
                if full_data_result:
                    full_error_dfs.append(
                        Pipeline.get_result_df(
                            program_instance=program_to_callable[program_name.value],
                            reference_data=full_data_result,
                ...
                for method in pipeline_input.sampling_methods:
                    df = pd.DataFrame(
                        columns=[
                            "sampling_fraction",
                            "sampling_method",
                            "relative_error_to_ref",
                            "relative_error_to_full",
                            "absolute_error_to_ref",
                            "absolute_error_to_full",
                        ]
                    )
                    sample_result = self.samples_info[fraction][method.value][
                        "programs_performance"
                    ][program_name.value]["result"]
                    full_result = self.samples_info[fraction][method.value][
                        "programs_performance"
                    ][program_name.value]["full_data_result"]
                    reference_result = self.samples_info[fraction][method.value][
                        "programs_performance"
                    ][program_name.value]["reference_data"]
                    ...
                    try:
                        df["relative_error_to_ref"] = program_class.get_error(
                            reference_data=reference_result,
                            test_data=sample_result,
                            use_relative_error=True,
                        )
                        df["absolute_error_to_ref"] = program_class.get_error(
                            reference_data=reference_result,
                            test_data=sample_result,
                            use_relative_error=False,
                        )
                    ...
                    try:
                        df["relative_error_to_full"] = program_class.get_error(
                            reference_data=full_result,
                            test_data=sample_result,
                            use_relative_error=True,
                        )
                        df["absolute_error_to_full"] = program_class.get_error(
                            reference_data=full_result,
                            test_data=sample_result,
                            use_relative_error=False,
                        )
