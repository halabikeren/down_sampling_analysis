...
def sample_data(
    full_data: t.List[SeqIO.SeqRecord],
    output_dir: str,
    ...
    if required_data_size > len(full_data):
        raise ValueError(
            f"full data size {len(full_data)} cannot be down-sampled to {required_data_size}"
        )
    ...
    for i in range(1, num_of_repeats + 1):
        ...
        if not os.path.exists(output_path):
            convert_names(
                translator_path=f"{output_dir}/sample_{i}/new_to_old_names_map.pickle",
...
def prepare_data(
    ...
    if sample_input:
        ...
        if not os.path.exists(output_dir):
            sampled_data_paths = sample_data(
                full_data=full_data,
            ...
            logger.info(
                f"{num_of_repeats} samples of size {required_data_size} generated successfully"
            )
    else:
    ...
    for input_path in sampled_data_paths:
        if additional_program_parameters and os.path.exists(
            additional_program_parameters
        ):
        ...
        if not os.path.exists(program_output_path):
            ...
            completion_validator_path = run_program(
                ...
                sequence_data_path=input_path,
                alignment_path=alignment_path,
                sequence_data_type=sequence_data_type,
                program_output_path=program_output_path,
                additional_params=additional_program_parameters,
            )
        sample_to_output[input_path] = {
            "alignment_path": alignment_path,
            "program_output_path": program_output_path,
    ...
    for input_path in sample_to_output:
        if "completion_validator_path" in sample_to_output[input_path]:
            while not os.path.exists(
                sample_to_output[input_path]["completion_validator_path"]
            ):
                sleep(10)
    ...
    if additional_simulation_parameters and os.path.exists(
        additional_simulation_parameters
    ):
    ...
    for input_path in sample_to_output:
        ...
        output = program_to_exec.parse_output(
            output_path=sample_to_output[input_path]["program_output_path"],
            job_output_dir=sample_to_output[input_path]["job_output_dir"],
        )
        additional_simulation_parameters[
            "simulations_output_dir"
        ] = f"{os.path.dirname(input_path)}/simulations/"
        os.makedirs(
            additional_simulation_parameters["simulations_output_dir"], exist_ok=True
        )
        ...
        logger.info(
            f"parsing complete, json written to {os.path.dirname(input_path)}/simulations.json"
        )
