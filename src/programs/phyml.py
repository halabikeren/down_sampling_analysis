...
class PhyML(Program):
    ...
    def set_command(
        ...
        program_output_dir = (
            output_dir
            if not parallelize
            else output_dir.replace(os.environ["container_data_dir"], cluster_data_dir)
        )
    ...
    def parse_phyml_stats(input_path: str) -> t.Dict[str, t.Any]:
        ...
        tree_size_regex = re.compile(
            "Tree size\:\s*(\d*\.?\d*)", re.MULTILINE | re.DOTALL
        )
        stats["tree_length"] = float(tree_size_regex.search(input_content).group(1))
        alpha_param_regex = re.compile(
            "Gamma shape parameter\:\s*(\d*\.?\d*)", re.MULTILINE | re.DOTALL
        )
        stats["alpha"] = float(alpha_param_regex.search(input_content).group(1))
        relative_rates_regex = re.compile(
            "Relative rate in class\s(\d*)\:\s*(\d*\.?\d*)", re.MULTILINE | re.DOTALL
        )
        stats["relative_rates"] = {
            int(match.group(1)): float(match.group(2))
            for match in relative_rates_regex.finditer(input_content)
        }
        states_frequencies_regex = re.compile(
            "\s-\sf\((\w)\)=\s*(\d*\.?\d*)", re.MULTILINE | re.DOTALL
        )
        stats["states_frequencies"] = {
            match.group(1): float(match.group(2))
            for match in states_frequencies_regex.finditer(input_content)
        }
        substitution_model_regex = re.compile(
            "Model of nucleotides substitution\:\s*(.*?)\n", re.DOTALL
        )
        stats["substitution_model"] = substitution_model_regex.search(
            input_content
        ).group(1)
        substitution_rates_regex = re.compile(
        ...
        version_regex = re.compile(
            "Version\:\s*(\d*\.?\d*\.?\d*)", re.MULTILINE | re.DOTALL
        )
        stats["phyml_version"] = version_regex.search(input_content).group(1)
        duration_regex = re.compile(
            "Time used\:\s*(\d*)h(\d*)m(\d*)s", re.MULTILINE | re.DOTALL
        )
        stats["duration"] = (
            float(duration_regex.search(input_content).group(1)) * 60
            + float(duration_regex.search(input_content).group(2))
            + float(duration_regex.search(input_content).group(3)) / 60
        )
        return stats
    ...
    def write_output_to_simulation_pipeline_json(
        ...
        simulation_input_parameters = {
            parameter: program_output[parameter]
            for parameter in [
                "substitution_model",
        ...
        simulation_input_parameters.update(
            {
                "tree_random": False,
