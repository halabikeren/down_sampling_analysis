...
class Sampler:
    ...
    saved_sequence: t.Optional[
        SeqIO.SeqRecord
    ] = None  # this sequence will appear in every sample
    ...
    def __init__(
        ...
        if self.exclude_a_ref_sequence:
            ...
            self.sequences_path = sequence_data_path.replace(
                ".fas", f"_without_reference_{self.saved_sequence.name}.fas"
            )
        else:
    ...
    def write_sample(
        self, k: int, aux_dir: str, output_path: str, **kwargs
